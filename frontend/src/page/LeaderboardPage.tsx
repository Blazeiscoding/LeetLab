...
import { IconCalendar, IconRefresh, IconTrendingUp, IconTrophy } from '@tabler/icons-react';
...
const getSolvedBreakdown = (entry: LeaderboardEntry): SolvedBreakdown => {
  if (typeof entry.problemsSolved === "number") {
    return (
      entry.problemsSolvedByDifficulty ?? {
        EASY: entry.easyCount ?? 0,
        MEDIUM: entry.mediumCount ?? 0,
        HARD: entry.hardCount ?? 0,
      }
    );
  }
...
const getEntryName = (entry: LeaderboardEntry): string => {
  return entry.user?.name || entry.user?.email || entry.userName || entry.userEmail || "User";
};

const getEntryScore = (entry: LeaderboardEntry): number => entry.totalScore ?? entry.score ?? 0;

const getEntrySolvedCount = (entry: LeaderboardEntry): number => {
  if (typeof entry.totalProblemsSolved === "number") {
    return entry.totalProblemsSolved;
  }
  ...
  const breakdown = getSolvedBreakdown(entry);
                ...
                <div className="relative mb-2">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gray-300 to-gray-400 p-1 shadow-lg">
                    <img 
                      ...
                      onError={(e) => {
                        e.currentTarget.src = getUserAvatar({ name: leaderboard[1]?.user?.name || 'U' });
                      }}
                    />
                  </div>
                ...
                <div className="relative mb-2">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-500 p-1 shadow-xl ring-4 ring-yellow-300/50">
                    <img 
                      ...
                      className="w-full h-full rounded-full object-cover bg-base-100"
                      onError={(e) => {
                        e.currentTarget.src = getUserAvatar({ name: leaderboard[0]?.user?.name || 'U' });
                      }}
                    />
                  </div>
                ...
                <div className="relative mb-2">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-amber-600 to-amber-700 p-1 shadow-lg">
                    <img 
                      ...
                      className="w-full h-full rounded-full object-cover bg-base-100"
                      onError={(e) => {
                        e.currentTarget.src = getUserAvatar({ name: leaderboard[2]?.user?.name || 'U' });
                      }}
                    />
                  </div>
                    ...
                    <AnimatePresence>
                      {leaderboard.map((entry, idx) => {
                        ...
                        const entryUser = entry.user || {
                          name: entryName,
                          email: entry.userEmail || entryName,
                            ...
                            <div className="flex items-center gap-3">
                              <div className="avatar">
                                <div className="w-10 h-10 rounded-full">
                                  <img 
                                    src={getUserAvatar(entryUser)}
                                    alt={entryName}
                                    className="object-cover"
                                    onError={(e) => {
                                      e.currentTarget.src = getUserAvatar({ name: entryName || 'U' });
                                    }}
                                  />
                                </div>
                              </div>
