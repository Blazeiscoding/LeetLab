...
const ProfilePage = () => {
  ...
  const getStats = () => {
    ...
    const difficultyBreakdown = solvedProblems.reduce<Record<string, number>>((acc, problem) => {
    ...
    const languageBreakdown = submissions.reduce<Record<string, number>>((acc, submission) => {
    ...
    <div className="min-h-screen bg-base-200/30">
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <div className="flex overflow-x-auto pb-4 lg:pb-0 gap-2 mb-8 no-scrollbar">
          ...
          ] as Array<{ id: ActiveTab; label: string; icon: typeof IconLayout }>).map(({ id, label, icon: Icon }) => (
            <button
              key={id}
              ...
              <div className="card-body">
                ...
                <div className="space-y-4">
                  {Object.entries(stats.languageBreakdown).length === 0 ? (
                    ...
                    (Object.entries(stats.languageBreakdown) as Array<[string, number]>)
                      .sort(([, a], [, b]) => b - a)
                      .map(([language, count]) => {
