...
import { CodeSnippets, Difficulty, Example, Problem } from "../types";

// ✅ Lazy load Monaco Editor - Only loads when needed
const Editor = lazy(() => import("@monaco-editor/react"));

type Language = "JAVASCRIPT" | "PYTHON" | "JAVA";
type ActiveTab = "description" | "hints" | "output" | "custom";

type ProblemDetailData = Omit<Problem, "hints"> & {
...
const ProblemDetailPage = () => {
  ...
  const defaultCodeSnippet = useMemo(() => {
    const snippets: CodeSnippets | undefined = problem?.codeSnippet || problem?.codeSnippets;
    return snippets?.[selectedLanguage] || "";
  ...
  const processedHints = useMemo(() => {
    ...
    try {
      if (typeof problem.hints === 'string') {
        ...
        return problem.hints.filter((hint): hint is string => typeof hint === "string");
      }
    } catch (error) {
  ...
  const hideHint = useCallback((index: number) => {
  ...
  const runCode = useCallback(async () => {
    ...
    try {
      ...
      const responseData = response.data.results as ExecuteResponseData;
  ...
  const submitCode = useCallback(async () => {
    ...
    try {
      ...
      const submissionData = response.data.submission as ExecuteResponseData;
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (elem instanceof HTMLElement) {
                          elem.blur(); // Close dropdown after selection
                        }
                      }}
                    >
                  ...
                  <div className="space-y-6">
                    ...
                    {testResults.testCases?.length > 0 && (
                      <div className="grid grid-cols-1 gap-4">
                        ...
                        {testResults.testCases.map((testCase: ExecutionTestCase, index: number) => (
