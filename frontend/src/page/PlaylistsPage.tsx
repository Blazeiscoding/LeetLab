...
const PlaylistsPage = () => {
  ...
  const getProgressPercentage = (playlist: Playlist) => {
    ...
    // Count solved problems based on the solvedBy relation
  ...
  const getDifficultyStats = (playlist: Playlist) => {
    if (!playlist.problems || playlist.problems.length === 0) {
      return { easy: 0, medium: 0, hard: 0 };
    }

    const stats = { easy: 0, medium: 0, hard: 0 };
    playlist.problems.forEach((problemInPlaylist: ProblemInPlaylist) => {
      const difficulty = problemInPlaylist.problem?.difficulty?.toLowerCase();
      ...
      {playlists.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {playlists.map((playlist) => {
            const progressPercentage = getProgressPercentage(playlist);
            const difficultyStats = getDifficultyStats(playlist);
            const totalProblems = playlist.problems?.length || 0;

            return (
              <div
                key={playlist.id}
                className="card bg-base-100 shadow-xl hover:shadow-2xl transition-shadow border border-gray-200"
              >
