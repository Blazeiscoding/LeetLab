...
const OTPLogin = ({ email: initialEmail, onBackToLogin, onSuccess }: OTPLoginProps) => {
  const [step, setStep] = useState<LoginStep>(initialEmail ? "otp" : "email");
  const [email, setEmail] = useState(initialEmail || "");
  const [timer, setTimer] = useState(0);
  const [remainingAttempts, setRemainingAttempts] = useState(5);
  const [, setOtpExpiry] = useState<string | null>(null);
    ...
    try {
      ...
      if (result.success) {
        ...
        if (errorMessage.includes("not found")) {
          ...
          setEmailError("email", { message: errorMessage });
        }
      }
    } catch (error) {
